...
public class InputFileFinder {
  ...
  private class FileCollector extends SimpleFileVisitor<Path> {
    ...
    public FileVisitResult visitFile(final Path file, BasicFileAttributes attrs) throws IOException {
      Path relativeFilePath = baseDir.relativize(file);
      boolean isSrc = srcMatcher.matches(file) || srcMatcher.matches(relativeFilePath);
      boolean isExcluded = excludeMatcher.matches(file) || excludeMatcher.matches(relativeFilePath);
      if (isSrc && !isExcluded) {
        boolean isTest = testsMatcher.matches(file) || testsMatcher.matches(relativeFilePath);
